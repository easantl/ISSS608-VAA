...
# Plot
ggraph(graph_influence, layout = "nicely") +
...
influence_types <- c("DirectlySamples", "InStyleOf",
...
graph_sub <- graph %>%
  ...
  filter(
    ...
    (`Edge Type` %in% influence_types & from %in% layer1_targets) |
    (`Edge Type` %in% creator_types & to %in% layer2_targets)
  )


used_nodes <- graph_sub %>%
  activate(edges) %>%
  as_tibble() %>%
  select(from, to) %>%
  unlist() %>%
